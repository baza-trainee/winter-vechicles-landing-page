...
export const ImgShown = () => {
  ...
  const getImageBasedOnWindowSize = () => {
    const windowWidth = window.innerWidth;

    if (windowWidth <= 767) {
      return window.devicePixelRatio > 1 ? imgMob2x : imgMob1x;
    } else if (windowWidth <= 1279) {
      return window.devicePixelRatio > 1 ? imgTab2x : imgTab1x;
    } else {
      return window.devicePixelRatio > 1 ? imgDesk2x : imgDesk1x;
    }
  };
